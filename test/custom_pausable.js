...
contract('Custom Pausable', function(accounts) {
  describe('Custom Pausable Ruleset', () => {
    ...
    it('must only be paused by an administrator.', async () => {
      await pausable.pause({from: accounts[2]}).should.be.rejectedWith(EVMRevert);

      await pausable.addAdmin(accounts[2]);
      await pausable.pause({from: accounts[2]});

      assert.equal(await pausable.paused(), true);
    });

    it('must only be unpaused by an administrator.', async () => {
      await pausable.pause({from: accounts[8]}).should.be.rejectedWith(EVMRevert);

      await pausable.addAdmin(accounts[8]);
      await pausable.pause({from: accounts[8]});
      assert.equal(await pausable.paused(), true);

      await pausable.removeAdmin(accounts[8]);
      await pausable.unpause({from: accounts[8]}).should.be.rejectedWith(EVMRevert);
      assert.equal(await pausable.paused(), true);

      await pausable.unpause({from: admins[1]});
      assert.equal(await pausable.paused(), false);
    });
  });
});
